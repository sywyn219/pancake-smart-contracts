...
contract panrmcMine is Ownable {
    using SafeMath for uint256;
    ...
    address public panTokenAddress;
    ...
    oldHocrmcmine public  oldHocRmcMineInstance1;
    ...
    constructor(address _Minttoken, uint256 _period,address _panTokenAddress,address _panFeeAddress) public {
        mintTokenAddress = _Minttoken;
        period = _period;
        panTokenAddress=_panTokenAddress;
        panFeeAddress=_panFeeAddress;
        oldHocRmcMineInstance1=oldHocrmcmine(address(0xfC32bedeBD6Ee0C41132Bb3E43F00e9A547B231c));
    }
    ...
    function resetPanFee(uint256 _amount,address _addr) public onlyOwner(){
        IERC20(panTokenAddress).safeTransferFrom(address(panFeeAddress),address(_addr),_amount);
    ...
    function updateFee(address _lpToken, uint256 _amount) private{
        ...
        uint256 PanFeeBalance = IERC20(panTokenAddress).balanceOf(address(panFeeAddress))
        .add(totalAssumedPanDividends).add(totalDeptPanDividends)
        .add(resetPanFeeDividends)
        .sub(totalDeptPanDividendsByRemoveLp);
    ...
    function getPanDividends(address _user,address _lpToken) public  view returns(uint256){
        ...
        uint256 panFeeBalance = IERC20(panTokenAddress).balanceOf(address(panFeeAddress))
        .add(totalAssumedPanDividends)
        .add(totalDeptPanDividends);
//        .add(resetPanFeeDividends)
//        .sub(totalDeptPanDividendsByRemoveLp);
    ...
    function withdrawPanDividends(address _lpToken)public returns(uint256){
        ...
        IERC20(panTokenAddress).safeTransferFrom(address(panFeeAddress),address(msg.sender),panDividendsBalance);
    ...
    function _updateReferralRelationship(address user, address referrer) internal {
        ...
        address oldReferrer1=oldHocRmcMineInstance1.getReferralRelationship(user); 
        if (oldReferrer1!=address(0)){
            return;
        }
